...
test("preview manifest covers exactly the approved action aliases and packed source frames", async () => {
  const manifest = JSON.parse(await fs.readFile(path.join(animationRoot, "manifest.json"), "utf8"));
  const packageJson = JSON.parse(await fs.readFile(path.join(projectRoot, "package.json"), "utf8"));

  assert.deepEqual(Object.keys(manifest.actions), previewActions.map(({ runtime }) => runtime));
